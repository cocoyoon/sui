...
pub async fn start_sui_tx_polling_task(
    ...
    metrics: Arc<DeepBookIndexerMetrics>,
) {
        ...
        let Ok(Ok(results)) = retry_with_max_elapsed_time!(
            sui_client.read_api().query_transaction_blocks(
                SuiTransactionBlockResponseQuery {
                    filter: Some(TransactionFilter::InputObject(DEEPBOOK_PACKAGE_ID)),
            ...
            error!("Failed to query deepbook transactions after retry");
            continue;
        };
        info!("Retrieved {} deepbook transactions", results.data.len());
